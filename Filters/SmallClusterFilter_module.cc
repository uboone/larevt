...
namespace cluster {
  ...
  private:
    ...
    const dataprov::DetectorProperties* detp = art::ServiceHandle<util::DetectorPropertiesService>()->getDetectorProperties(); ///< const ptr to det properties data provider
...
{ 
  ...
  mf::LogVerbatim("SmallClusterFilter") << " ++++ Hitsreceived received " 
					<< HitListHandle->size() << " +++++ ";
  ...
  mf::LogVerbatim("SmallClusterFilter") << "\nPassing with " 
					<< hitlistbyplane[fNPlanes-1].size() 
					<< " hit(s) on collection.\n";
